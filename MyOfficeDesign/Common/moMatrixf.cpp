...
// now compile up Matrix via Matrixf
...
Matrixf::Matrixf( value_type a00, value_type a01, value_type a02, value_type a03,
...
void Matrixf::set( value_type a00, value_type a01, value_type a02, value_type a03,
				  value_type a10, value_type a11, value_type a12, value_type a13,
				  value_type a20, value_type a21, value_type a22, value_type a23,
				  value_type a30, value_type a31, value_type a32, value_type a33)
{
...
void Matrixf::setRotate(const Quat& q)
{
	...
	else
	{
		...
		// in left-handed rotations, which is contrary to OpenGL and 's 
...
Quat Matrixf::getRotate() const
{
	...
	QW = 0.5 * sqrt( maximum( 0.0, 1.0 + _mat[0][0] + _mat[1][1] + _mat[2][2] ) );
	QX = 0.5 * sqrt( maximum( 0.0, 1.0 + _mat[0][0] - _mat[1][1] - _mat[2][2] ) );
	QY = 0.5 * sqrt( maximum( 0.0, 1.0 - _mat[0][0] + _mat[1][1] - _mat[2][2] ) );
	QZ = 0.5 * sqrt( maximum( 0.0, 1.0 - _mat[0][0] - _mat[1][1] + _mat[2][2] ) );
	...
	QX = QX * signOrZero(  _mat[1][2] - _mat[2][1]) ;
	QY = QY * signOrZero(  _mat[2][0] - _mat[0][2]) ;
	QZ = QZ * signOrZero(  _mat[0][1] - _mat[1][0]) ;
#else
	QX = QX * sign(  _mat[1][2] - _mat[2][1]) ;
	QY = QY * sign(  _mat[2][0] - _mat[0][2]) ;
	QZ = QZ * sign(  _mat[0][1] - _mat[1][0]) ;
#endif
...
int Matrixf::compare(const Matrixf& m) const
{
	const Matrixf::value_type* lhs = reinterpret_cast<const Matrixf::value_type*>(_mat);
	const Matrixf::value_type* end_lhs = lhs+16;
	const Matrixf::value_type* rhs = reinterpret_cast<const Matrixf::value_type*>(m._mat);
...
void Matrixf::setTrans( value_type tx, value_type ty, value_type tz )
{
	_mat[3][0] = tx;
	_mat[3][1] = ty;
	_mat[3][2] = tz;
...
void Matrixf::setTrans( const Vec3f& v )
{
	_mat[3][0] = v[0];
	_mat[3][1] = v[1];
	_mat[3][2] = v[2];
}
void Matrixf::setTrans( const Vec3d& v )
{
	_mat[3][0] = v[0];
	_mat[3][1] = v[1];
	_mat[3][2] = v[2];
...
void Matrixf::makeScale( const Vec3f& v )
{
	makeScale(v[0], v[1], v[2] );
}

void Matrixf::makeScale( const Vec3d& v )
{
	makeScale(v[0], v[1], v[2] );
}

void Matrixf::makeScale( value_type x, value_type y, value_type z )
{
	SET_ROW(0,    x, 0, 0, 0 )
		SET_ROW(1,    0, y, 0, 0 )
		SET_ROW(2,    0, 0, z, 0 )
...
void Matrixf::makeTranslate( const Vec3f& v )
{
	makeTranslate( v[0], v[1], v[2] );
}

void Matrixf::makeTranslate( const Vec3d& v )
{
	makeTranslate( v[0], v[1], v[2] );
}

void Matrixf::makeTranslate( value_type x, value_type y, value_type z )
{
	SET_ROW(0,    1, 0, 0, 0 )
		SET_ROW(1,    0, 1, 0, 0 )
		SET_ROW(2,    0, 0, 1, 0 )
...
void Matrixf::makeRotate( const Vec3f& from, const Vec3f& to )
{
	makeIdentity();
...
void Matrixf::makeRotate( const Vec3d& from, const Vec3d& to )
{
	makeIdentity();
...
void Matrixf::makeRotate( value_type angle, const Vec3f& axis )
{
	makeIdentity();
...
void Matrixf::makeRotate( value_type angle, const Vec3d& axis )
{
	makeIdentity();
...
void Matrixf::makeRotate( value_type angle, value_type x, value_type y, value_type z ) 
{
	makeIdentity();
...
void Matrixf::makeRotate( const Quat& quat )
{
	makeIdentity();
...
void Matrixf::makeRotate( value_type angle1, const Vec3f& axis1, 
...
void Matrixf::makeRotate( value_type angle1, const Vec3d& axis1, 
...
void Matrixf::mult( const Matrixf& lhs, const Matrixf& rhs )
{   
	if (&lhs==this)
	{
		postMult(rhs);
...
void Matrixf::preMult( const Matrixf& other )
{
	// brute force method requiring a copy
	//Matrixf tmp(other* *this);
	// *this = tmp;
...
void Matrixf::postMult( const Matrixf& other )
{
	// brute force method requiring a copy
	//Matrixf tmp(*this * other);
	// *this = tmp;
...
void Matrixf::orthoNormalize(const Matrixf& rhs)
{
...
bool Matrixf::invert_4x3( const Matrixf& mat )
{
	if (&mat==this)
	{
		Matrixf tm(mat);
...
bool Matrixf::invert_4x4( const Matrixf& mat )
{
	if (&mat==this) {
		Matrixf tm(mat);
...
{
	// note transpose of Matrixf wr.t OpenGL documentation, since the  use post multiplication rather than pre.
...
bool Matrixf::getOrtho(double& left, double& right,
					   double& bottom, double& top,
...
bool Matrixf::getFrustum(double& left, double& right,
						 double& bottom, double& top,
						 double& zNear, double& zFar) const
{
...
bool Matrixf::getPerspective(double& fovy,double& aspectRatio,
							 double& zNear, double& zFar) const
{
	double right  =  0.0;
	double left   =  0.0;
...
void Matrixf::makeLookAt(const Vec3d& eye,const Vec3d& center,const Vec3d& up)
{
	Vec3d f(center-eye);
	f.normalize();
	Vec3d s(f^up);
...
void Matrixf::makeLookAtCenter(const Vec3d& eye,const Vec3d& center,const Vec3d& up)
{
	Vec3d _eye,_center;
	_eye = eye - center;
	_center = Vec3d(0,0,0);
	makeLookAt(_eye,_center,up);
	//makeLookAt(eye,center,up);
}

void Matrixf::getLookAt(Vec3f& eye,Vec3f& center,Vec3f& up,value_type lookDistance) const
{
	Matrixf inv;
	inv.invert(*this);
...
void Matrixf::getLookAt(Vec3d& eye,Vec3d& center,Vec3d& up,value_type lookDistance) const
{
	Matrixf inv;
	inv.invert(*this);
